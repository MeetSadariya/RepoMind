...
// CORS for SaaS: configurable front-end
...
async function runJobPipeline(jobId) {
  ...
  try {
    ...
    // 5) Generate docs (AI if available, otherwise fallback)
    job.status = "ai-generating";
    const useAI = Boolean(process.env.GEMINI_API_KEY);
    const docs = useAI
      ? await generateAIDocs(fileContents, (current, total, filePath) => {
          console.log(`AI docs ${current}/${total} for ${filePath}`);
        })
      : generateFakeDocs(fileContents);
    job.docs = docs;
...
app.get("/jobs/:id/docs-list", (req,res) => {
    ...
    if(!job.docsDir || !fs.existsSync(job.docsDir)){
        return res.status(400).json({
            error: "Docs not found. Make sure /generate-docs and /save-docs were called",
        });
    }
